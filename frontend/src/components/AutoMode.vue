<template>
  <div>
    <button class="btn-today-count" @click="fetchDataByName()">点我查看今日已入库数据</button>
    <br/>
    ...
    <div v-if="polling" class="polling-section">
      <div class="polling-status">🔄 正在每10秒自动获取下载任务...（第{{ pollCount }}次）</div>
    ...
    <div class="info-section" v-if="downloadUrl">
      ...
      <div class="info-item">
        ...
        <div class="info-value">{{ originalCurrentTargetNum }}</div>
      </div>
    </div>
    ...
    <button class="btn-event" @click="queryEventWithFullMsg()" :disabled="eventLoading" style="margin-left:5px; ">
      查看JSON
    </button>
...
<!--    </div>-->
  ...
  <KeepAlive include="AutoMode" :max="5">
    <component :is="div" />
  </KeepAlive>
</template>
<style scoped>
