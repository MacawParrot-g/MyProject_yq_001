<template>
  <div class="app-wrapper">
    <h1>"我看行"测试数据可视化系统3.0</h1>
    <div class="subtitle">仅限自用，禁止分享</div>
    ...
    <AutoMode v-if="mode === 'auto'" @error="setError" />
    <ManualMode v-if="mode === 'manual'" @error="setError" />
    <ExportMode v-if="mode === 'export'" @error="setError" />
    <QRCodeBuilderByMan v-if="mode==='qrcode'" @error="setError"/>
    <DataVisitable v-if="mode ==='data'" @error="setError"/>
  </div>
</template>
