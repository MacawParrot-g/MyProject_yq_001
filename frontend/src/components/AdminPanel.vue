...
async function fetchData(resetPage = false) {
  ...
  queried.value = true
  try {
    const json = await fetchRecordListByDate(
  ...
  <div class="admin-page">
    ...
    <div v-if="activeTab === 'data'" class="admin-section">
      <div class="filter-card">
        ...
        <div class="filter-section filter-search-row">
          <div class="filter-field">
            <label class="field-label">测试人</label>
            <input v-model="recorderSearch" class="filter-input" placeholder="输入姓名" @keyup.enter="fetchData(true)" />
          </div>
          <button class="btn-query" @click="fetchData(true)" :disabled="loading">
            {{ loading ? '查询中...' : '🔍 查询' }}
          </button>
      ...
      <div v-if="queried && viewType" class="view-type-bar">
        <span class="view-type-label">当前视图</span>
        <strong>{{ viewTypeLabel[viewType] || viewType }}</strong>
        <span v-if="total > 0" class="view-type-count">共 {{ total }} 条 · 第 {{ currentPage }}/{{ totalPages }} 页</span>
      </div>
