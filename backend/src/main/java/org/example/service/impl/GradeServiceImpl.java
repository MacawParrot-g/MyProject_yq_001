...
public class GradeServiceImpl implements GradeService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Result deleteGrade(String url) {
        if (url == null || url.isEmpty()) return Result.fail("URL不能为空");
        try {
            int count = gradeMapper.deleteByUrl(url);
            if (count == 0) return Result.fail("该评级记录不存在");
            gradeRedisTemplate.opsForSet().remove(GRADE_URLS_KEY, url);
            gradeRedisTemplate.opsForHash().delete(GRADE_RECORD_KEY, url);
            log.info("✅ 评级记录已删除（MySQL+Redis DB3）: {}", url);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Result updateGrade(String url, String grade, String remark) {
        if (url == null || url.isEmpty()) return Result.fail("URL不能为空");
        if (grade == null || grade.isEmpty()) return Result.fail("请选择评级等级");
        try {
            Grade existing = gradeMapper.selectByUrl(url);
            ...
            log.info("✅ 评级记录已更新: URL={}, grade={}", url, grade);
