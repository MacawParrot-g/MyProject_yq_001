...
public class RecordOperateMessageProducer {
    ...
    public boolean sendDeleteMessage(String url) {
        try {
            RecordDeleteMessage message = new RecordDeleteMessage(url, System.currentTimeMillis());
            rabbitTemplate.convertAndSend(
                    RabbitMQConfig.DELETE_EXCHANGE,
                    RabbitMQConfig.DELETE_ROUTING_KEY,
                    message
            );
            log.info("删除消息已投递至RabbitMQ, URL: {}", url);
            return true;
        } catch (Exception e) {
            log.error("❌ 删除消息投递失败, URL: {}, 原因: {}", url, e.getMessage());
            return false;
        }
    }
}
